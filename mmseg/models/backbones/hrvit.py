...
# from ..builder import BACKBONES #注册器被我换成mmseg了。这样Backone可以穿init_cfg不用调用所谓的checkpoint函数，也不需要logging
...
class HRViT(BaseModule):
    ...
    def init_weights(self):
        if (isinstance(self.init_cfg, dict)
            ...
            load_ckp_model(self, self.init_cfg['checkpoint'])
            """
...
def load_ckp_model(model, model_file): #载入HRVit权重
    # load raw state_dict
    t_start = time.time()
