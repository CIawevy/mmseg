...
from ..utils import FeatureFusionModule as FFM
from ..utils import FeatureRectifyModule as FRM
import math
import time
...
class RGBXTransformer(BaseModule):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), norm_fuse=nn.BatchNorm2d,
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], stride0=4,stage_cfg=dict(stage='detection'),
                 init_cfg=None,raw:bool=True):
        super().__init__(init_cfg=init_cfg)
        self.num_classes = num_classes
        self.depths = depths
        self.raw=raw
        self.stage=stage_cfg['stage']
    ...
    def init_weights(self):
        if (isinstance(self.init_cfg, dict)
            ...
            load_dualpath_model(self,self.init_cfg['checkpoint'],self.raw)
            """
            mit-b2双流训练时使用 dual_path_modle做法是load一遍mitb2然后复制上面的一份给下面的block作为初始权重。
            如果是预训练整体则不需要这个改动,我在代码中使用raw直接控制跳过
            """

        elif self.init_cfg is not None: #不是pretrain参数就用mmseg其他初始化配置参数
            super().init_weights()
        else:
            #应该可以替代_init_weights在没有init_cfg时的功能。不用初始化两遍了出于保险还是不删除上面
...
def load_dualpath_model(model, model_file,raw:bool): #载入mit-b2权重 并分配给noise分支
    # load raw state_dict
    t_start = time.time()
